...
from Modelos.Algoritmos.Knn import KNNChurnPredictionModel
from Data.Clientes_Churn_Chile import Clientes_Churn

# Función para analizar los clientes en riesgo usando KNN
def analyze_risk_clients_knn(window: tk.Toplevel):
    df = pd.read_excel('Data/Clientes-Churn-Chile-Sky-22102024.xlsx')
    model = KNNChurnPredictionModel(df)
    result = model.analyze_risk_clients()
    table_window = FilterableSortableTable(window, "Clientes en riesgo de abandono (KNN)", "1200x600", '', custom_data=result)

# Crear la ventana y los elementos gráficos para el modelo KNN
def create_knn_top_level():
    total_rows_in_standar_window = 2
    height = 500
    window = Utilities.create_standar_top_level(
        "KNN - Vecinos más cercanos", 
        "Modelo KNN para predicción de Churn", 
        height,
        analyze_risk=True,
        analyze_risk_command=analyze_risk_clients_knn
    )
    ...
    def predict_churn_knn():
        try:
            ...
            messagebox.showinfo("Resultado de la Predicción (KNN)", result, parent=window)
    ...
    predict_button = tk.Button(window, text="Predecir Churn (KNN)", command=predict_churn_knn)
...
# Diccionario para el modelo KNN
KNN = {"label": "Usar el algoritmo KNN", "action": create_knn_top_level}
